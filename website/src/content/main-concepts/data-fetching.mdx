...
The Apollo Client is already pre-configured within the [`<ApplicationShell>`](/main-concepts/application-shell) to connect to the `/graphql` endpoint of the [Merchant Center API Gateway](/main-concepts/api-gatway). You don't need to configure it on your own, unless you need to connect to an **external** GraphQL API (see [Connecting to an external GraphQL API](#connecting-to-an-external-graphql-api)).
...
The SDK library is already pre-configured within the [`<ApplicationShell>`](/main-concepts/application-shell). You don't need to configure it on your own.

</Info>

## Fetching using the built-in SDK library

In the example below, we fetch a [Channel](https://docs.commercetools.com/http-api-projects-channels) from the commercetools platform HTTP API.
